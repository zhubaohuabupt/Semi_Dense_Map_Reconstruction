...
void TemplatedVocabulary<TDescriptor,F>::saveToBinaryFile(const std::string &filename) const {
  fstream f;
  f.open(filename.c_str(), ios_base::out|ios::binary);//以二进制输出的方式打开文件 输出指的是向文件里输出
  ...
  f.write((char*)&nb_nodes, sizeof(nb_nodes));//从缓存区nb_nodes中取sizeof位写入文件f
  ...
  for(size_t i=1; i<nb_nodes;i++) {
	...
	f.write((char*)node.descriptor.data, F::L);//L=32
